...
import { StatementUpload, type StatementType } from "@/features/import/components/StatementUpload";
import {
...
export default function Import() {
  ...
  const mapDbCategoryToLocal = (dbCat: DbCategory): Category => ({
    ...
    color: dbCat.color,
    icon: getIconByName(dbCat.icon || 'circle-dot'),
  });
  ...
  const handleUploadComplete = async (text: string, fileName: string, type: StatementType) => {
    ...
    try {
      // Usa a nova rota otimizada (5-10x mais rápida)
      const response = await apiRequest("POST", "/api/extract-transactions-fast", {
        text,
        fileName,
        statementType: type,
      });
      ...
      const transactions: ExtractedTransaction[] = data.transactions.map(
        (t: any, index: number) => {
          // Backend DEVE enviar categoryId (categorização é feita no servidor)
          // Busca categoria pelo ID retornado pelo servidor
          const dbMatch = dbCategories.find(c => c.id === t.categoryId);

          const category = dbMatch
            ? mapDbCategoryToLocal(dbMatch)
            : (defaultCategories.find(c => c.type === t.type) || defaultCategories[4]);

          if (!dbMatch && t.categoryId) {
            logger.warn(`Category ${t.categoryId} not found in database, using default`);
          }
